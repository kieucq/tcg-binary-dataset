...
def list_reanalysis_files(path: str) -> pd.DataFrame:
    files = glob.iglob(os.path.join(path, '**', '*.grib1'))
...
def load_grib1(path, filter_by_keys: dict) -> xr.Dataset:
...
def load_dataset(path: str) -> xr.Dataset:
    ...
    surface_ds = load_grib1(
        path, filter_by_keys=dict(typeOfLevel='surface'))
    surface_ds = surface_ds.rename_vars(dict(t='tsfc'))

    tropopause_ds = load_grib1(
        path, filter_by_keys=dict(typeOfLevel='tropopause'))
    tropopause_ds = tropopause_ds.rename_vars(dict(gh='htrp'))
    tropopause_ds = tropopause_ds.rename_vars(dict(u='utrp'))
    tropopause_ds = tropopause_ds.rename_vars(dict(v='vtrp'))
    tropopause_ds = tropopause_ds.rename_vars(dict(unknown='t1'))
    tropopause_ds = tropopause_ds.rename_vars(dict(pres='t2'))
    try:
       tropopause_ds = tropopause_ds.rename_vars(dict(tsfc='ttrp'))
    except Exception:  
       tropopause_ds = tropopause_ds.rename_vars(dict(t='ttrp'))

    #print(surface_ds)
    #quit()

    # Merge datasets.
    merged_ds = common_ds.merge(surface_ds).merge(tropopause_ds)
